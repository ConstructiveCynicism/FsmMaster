...
// of what FsmMasterPlugin itself did directly on the Silksong branch, moved onto a dedicated
...
internal class FsmMasterDriver : MonoBehaviour
{
    ...
    // clear it from, matching every other static handle on this branch (Harmony patches installed once
    // and never unpatched; see BO's no-hot-reload lifecycle note).
    ...
    private void Update()
    {
        ...
        // static flag - see that patch's own comment for why this branch can leave the postfix
    ...
    private void OnGUI()
    {
        ...
        // Both panels are freely draggable/resizable - the vignette needs the monitor panel's own
    ...
    private void BuildRightPanel()
    {
        ...
        _monitorPanel = new FsmMonitorPanel(_uiCommon, _variableTracker!);
    ...
    // memory, for instance. Filtering on gameObject.scene.IsValid() restores the same "only scene-
    // instantiated objects" restriction FindObjectsOfType already guaranteed, without losing inactive
    // ones.
    private static PlayMakerFSM[] FindAllPlayMakerFsms() =>
        Resources.FindObjectsOfTypeAll<PlayMakerFSM>().Where(fsm => fsm.gameObject.scene.IsValid()).ToArray();
    ...
    private Dictionary<string, List<PlayMakerFSM>> ApplyPersistedEditsForScene(string sceneName, PlayMakerFSM[] components)
    {
        ...
        if (!FsmMasterMod.Instance!.GlobalSettings.AutoLoadLastConfiguration)
        {
            return groups;
        }
