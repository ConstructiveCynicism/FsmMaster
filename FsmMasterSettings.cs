...
// How transition lines are drawn - see FsmGraphOverlay.DrawLineBufferGL (ported in a later phase).
// Thick: antialiased curved lines with arrowheads (most detailed, most expensive). Thin: the same
// curves drawn as hard-edged 1px lines with arrowheads. Straight: a plain straight segment between
// each transition's endpoints, no arrowhead (cheapest).
...
// How state/event boxes are drawn - see FsmGraphOverlay's chrome-gathering pass (ported in a later
// phase). Detailed: rounded corners, a border ring on every box, divider lines between the title and
// its transition rows. Standard: square corners, no border ring unless active/selected, no dividers.
...
public class FsmMasterGlobalSettings : IModSettings
{
    ...
    // scalar bool/int/float/string storage (confirmed by reflecting the patched Assembly-CSharp.dll -
    // StringValues/IntValues/BoolValues/FloatValues are its only backing dictionaries), so Color values
    // ride along on the framework's whole-object serialization of GlobalSettings instead, the same way
    // DebugMod's own GlobalSettings.binds field (a SerializableIntDictionary) does.
